...
1. **ARM64 `tunnel.dll`** — Go+CGO cross-build needing the llvm-mingw aarch64 toolchain (outstanding since 3.9.0). Without it there's no valid `MasselGUARD-arm64.zip`. **Top blocker.**
2. **Verify `wireguard.dll` (wireguard-nt) licence + prebuilt-DLL redistribution terms** (§5.1) — the one open licensing item; `tunnel.dll` is MIT/clean.
...
4. **Tag/release `4.0.0`** with **both** arch zips (needs 1).
