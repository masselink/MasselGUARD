...
| Themes | `<exedir>\shared_themes\<folder>\theme.json` (shipped/downloaded, read-only but deletable+copyable: blueongrey="Blue on grey", highcontrast="High Contrast" (WCAG AAA), glass="Glass" (translucent); discovered from disk by `ThemeManager.SharedThemeNames()`) + `%APPDATA%\MasselGUARD\custom_themes\` (user; migrated from the pre-3.7 `themes\` folder by `ThemeManager.MigrateLegacyUserThemes`). Only the virtual `__system__` (Windows colours) theme is embedded in code and read-only (`IsBuiltinTheme`). Builder groups: Built-in (System, read-only, no delete) / Shared (read-only, deletable, Duplicate-to-edit) / Custom (editable). The Dark/Light pill previews live for read-only themes too (`ApplyDraftLive` runs regardless of `_readOnly`). |
