...
Current version: **4.2.0 — Resolving Raven**
...
- **DNS automation (Model C, tunnel-independent)** — full design in [`docs/DnsAutomation-Design.md`](docs/DnsAutomation-Design.md). Applies a **DNS resolver per network even with no tunnel active** ("on Wi-Fi X → DNS Y", plain or DoH). A **parallel axis** to the tunnel rule: `DnsPolicy.Evaluate` (pure, CLI-shared, self-tested) resolves `DnsResult(None|Apply|Automatic)` with the same precedence as `EvaluateWifi` (off→open→SSID→trusted→schedule→default); `RuleEngine.EvaluateDns` wraps it, and `EvaluateWifi`/`EvaluateSchedules` skip **DNS-only rules** (`DnsPolicy.IsDnsOnly` — empty Tunnel + a `DnsProfileId`) so they never read as "disconnect". `MainViewModel.ApplyWifiState` runs it alongside the tunnel action; `DnsService` (GUI-only) writes per-interface DNS via **netsh** (DoH via `netsh dns add encryption` autoupgrade, Win11-gated, `RequireEncryption`=fail-closed), keyed by `WiFiService.CurrentInterfaceGuid`, honouring `AppConfig.DnsAddressFamilies`. **Resolver ownership (§6):** while any tunnel is active its own DNS/NRPT supersedes, so DNS actions are held and re-asserted on the tunnel's falling edge (`_tunnelOwnsDns` in `RefreshTunnelStatus`). **Lifecycle:** original DNS is snapshotted to `%APPDATA%\MasselGUARD\dns_state.json` before the first override and restored on exit (`App.OnExit`→`MainViewModel.RestoreDnsOverrides`) and at startup after a crash (`RecoverDnsFromPreviousRun`). Model: `Models/DnsProfile.cs` (+ `BuiltInPresets`), `TunnelRule.DnsProfileId`, `AppConfig.{DnsAutomationEnabled,DnsProfiles,DefaultDnsProfileId,OpenWifiDnsProfileId,DnsAddressFamilies}`. UI on its own **Settings → DNS** tab (`PageDns`/`TabBtnDns`; toggle + default/open/families + profiles manager via `Views/DnsProfileEditor.cs`; `RefreshDnsControls` runs on the "Dns" tab) and an optional DNS picker in `RuleDialog`. CLI: `dns status` (read-only). **`DnsPolicy.cs` is shared — listed in `MasselGUARDcli.csproj`** (`DnsService.cs` is GUI-only). Verified by `MasselGUARDcli selftest` (DnsPolicy precedence, 19 cases).
- **CLI elevation** — `MasselGUARDcli` requires admin, but the informational commands `help`/`version`/`selftest`/`dns` (+ aliases) bypass the gate in `MasselGUARDcli/Program.cs` (`IsNonElevatedCommand`) so they run in any terminal. Anything touching the driver/service still elevates. (`dns` is read-only in this release; a future write subcommand must re-gate.)
- **Cloud-synced-location guard** — a local tunnel is hosted by a Windows service running as **LocalSystem**, which cannot read files inside a per-user cloud-synced folder (OneDrive) — the tunnel then dies with **"Element not found"** (the exe, the temp `.conf` under `<exedir>\tunnels\temp\`, and the native DLLs are all unreadable to LocalSystem there). `MainWindow.WarnIfCloudSyncedLocation` (deferred from `OnLoaded`) detects this via `IsCloudSyncedPath` (the `OneDrive*` env roots, an "OneDrive" path segment, or a reparse-point/placeholder up the tree): it **always logs** a warning and shows a themed dialog (`CloudSyncTitle`/`CloudSyncWarning`/`CloudSyncLog`, localized ×12) **only when ≥1 local tunnel exists** (companion-only setups are unaffected). Fix is environmental: run from a normal local folder (e.g. `C:\MasselGUARD` or Program Files).
- **Native DLL build (`tunnelbuild\`)** — `tunnelbuild.bat <arch|all> [force]` orchestrates `get-wireguard-dlls.ps1`, which **downloads** `wireguard.dll` (wireguard-NT, per-arch, verified by size + PE machine) and **builds** `tunnel.dll` from a fresh `wireguard-windows` clone via its own **hermetic** `build.bat` (which self-provisions Go + llvm-mingw into `.deps` — **no system Go/compiler needed**, only git + curl + tar). Cache-first: it only (re)builds a missing/wrong-arch DLL; `force` sets the cached DLL aside (`.forcebak`, restored on failure) and rebuilds. A patch (`Repair-WgToolsDownload`) rewrites the one wireguard-tools download from the flaky `git.zx2c4.com` cgit snapshot to the GitHub mirror (dynamic SHA256) since curl-schannel drops that host. `tunnelbuild.bat check [install]` validates git/curl/tar + network (and can winget-install git). Hashing/zip use `.NET`/`tar` (not `Get-FileHash`/`Expand-Archive`, which can be unavailable in locked-down hosts). **The DLLs must not live in a OneDrive path at runtime** (see the cloud-location guard above).
...
Services/
  ...
  RuleEngine.cs             # WiFi/schedule/trusted rule precedence (EvaluateWifi/EvaluateSchedules); trusted rules are directional (TunnelRule.TrustedWhen); EvaluateDns wraps DnsPolicy
  DnsPolicy.cs              # PURE DNS-rule precedence (EvaluateDns axis) + IsWithinSchedule + IsDnsOnly. CLI-shared (selftest); add to CLI csproj!
  DnsService.cs             # Applies/reverts per-interface DNS (netsh; DoH via netsh dns add encryption) + snapshot/restore. GUI-only
