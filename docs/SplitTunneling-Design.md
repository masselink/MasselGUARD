...
5. ✅ **DONE — Editor UI.** **Split** tab in `TunnelConfigDialog` (local-tunnel editor only — `TunnelMetadataDialog` untouched), positioned **between Fields and Options**: 3 mode radios (Off/Exclude/Include) + a ranges `TextBox` (one CIDR per line, enabled only when a mode is active, validated on save via `IsValidCidr` → `SplitInvalidRange` message + tab focus). Threaded through `existingSplitMode`/`existingSplitRanges`/`existingSplitApps` ctor args + `ResultSplitMode`/`ResultSplitRanges`, wired in `MainWindow` add/edit (SplitApps preserved untouched on edit). 11 lang keys ×12 (parity 739). **The disabled "Apps" preview box was removed (2026-09-09)** — per-app split is a later 4.x, so no placeholder UI ships now; the `SplitAppsLabel`/`SplitAppsComingSoon` keys remain in the lang files, ready for when it lands. **Live effective-AllowedIPs preview added (2026-09-09):** a read-only `SplitPreviewBox` shows `Base:` (from the Fields-tab AllowedIPs) and `Effective:` (computed via `CidrMath.ComputeEffectiveAllowedIPs`), recomputed on mode change, ranges `TextChanged`, and `TabSplit` `GotFocus` (so Fields-tab base edits are reflected). 3 lang keys ×12 (`SplitPreviewLabel`/`Base`/`Effective`), parity 742. Both builds clean.
...
**Follow-up fix (2026-09-09):** the editor **Raw** tab renders via `DialogSettings()` (live controls), which originally didn't include split — so split showed in a toolbar export (`TunnelSettings.From(StoredTunnel)`) but not in the Raw view. `DialogSettings()` now snapshots `SplitMode`/`SplitRanges` too (same off/empty-omit convention as `From`), so the Raw view and a toolbar export match, and Raw→save round-trips split (`ParseImportText` → `ApplySettingsToControls`).
